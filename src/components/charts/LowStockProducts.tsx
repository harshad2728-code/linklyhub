...
import { Zap, Package, ArrowUpCircle } from "lucide-react";
...
const StockPill: React.FC<{ stock: number }> = ({ stock }) => {
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-0.5 rounded-full border ${colorClass}`}>
...
const LowStockProducts: React.FC = () => {
  const lowStockItems: Product[] = [
    { id: "1", name: "Wireless Mouse (Ergo-X)", image: "/images/product_img/mouse.jpg", stock: 3 },
    { id: "2", name: "Aura Smartwatch v3", image: "/images/product_img/watch.jpg", stock: 2 },
    { id: "3", name: "Premium Cotton T-Shirt (M)", image: "/images/product_img/Tshirt.jpg", stock: 5 },
    { id: "4", name: "USB-C Fast Charger (65W)", image: "/images/product_img/charger.jpg", stock: 1 },
  ];
              ...
              <tr key={product.id} className="hover:bg-gray-50 dark:hover:bg-[#253049] transition duration-150">
                <td className="py-3 pr-3 whitespace-nowrap">
                  <div className="flex items-center gap-3">
                    <Image
                      src={product.image || "/images/product_img/default.jpg"} // fallback image
                      alt={product.name}
                      width={48}
                      height={48}
                      className="rounded-lg object-cover border border-gray-200 dark:border-gray-600 shadow-sm"
                      onError={(e) => {
                        (e.target as HTMLImageElement).src = "/images/product_img/default.jpg"; // fallback if 404
                      }}
                    />
